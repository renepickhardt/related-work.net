...
public class SuggestTree<V> {
	...
	private static class Node implements SuggestionList {
		...
		public String get(int index) {
			return (index == 0) ? first : list[index - 1];
		}
	...
	public SuggestTree(int k, final Comparator<V> comparator) {
		...
		maxArrayLength = k - 1;
		rankComparator = new Comparator<Map.Entry<String, V>>() {
	...
	public void build(Map<String, V> map) {
		root = newRoot = null;
		nodeCount = arrayCount = totalArrayLength = 0;
		buildTernarySearchTree(map);
		buildSuggestionLists(map);
		root = newRoot;
	}
	...
	public String sizeInfo() {
		return "nodeCount=" + nodeCount + ", arrayCount=" + arrayCount
				+ ", totalArrayLength=" + totalArrayLength;
	}
	...
	private void buildTernarySearchTree(Map<String, V> map) {
		...
		if (suggestions.length > 0 && suggestions[0].isEmpty())
			throw new IllegalArgumentException();
		buildBalancedTree(suggestions, 0, suggestions.length);
	}

	private void buildBalancedTree(String[] suggestions, int left, int right) {
		if (left < right) {
			int mid = (left + right) / 2;
			newRoot = insert(suggestions[mid], 0, newRoot);
	...
	private Node insert(String suggestion, int i, Node n) {
		if (n == null) {
			...
			n.left = insert(suggestion, i, n.left);
		else if (suggestion.charAt(i) > n.first.charAt(i))
			n.right = insert(suggestion, i, n.right);
		else {
			while (++i < n.end) {
			...
			if (n.count < maxArrayLength)
				n.count++;
			if (i < suggestion.length())
				n.mid = insert(suggestion, i, n.mid);
		}
		return n;
	}
	...
	private void buildSuggestionLists(Map<String, V> map) {
		...
		Arrays.sort(a, rankComparator);
		for (Map.Entry<String, V> e : a)
			appendToLists(e.getKey());
	}
			...
			if (suggestion.charAt(i) < n.first.charAt(i))
				...
				if (n.list == null) {
					...
					if (n.count == 0)
						...
						arrayCount++;
						totalArrayLength += n.count;
						n.count = 0;
					}
				} else if (n.count < maxArrayLength)
